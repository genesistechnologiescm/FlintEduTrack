...
export function GateBanner({ initial }: { initial: GateState }) {
    ...
    return (
      <div
        className={`mx-auto mb-4 flex max-w-[560px] items-center justify-between gap-3 rounded-2xl border px-4 py-3 ${
          state.onTime ? "border-success/25 bg-success/5" : "border-amber-500/30 bg-warn-bg"
        }`}
      >
        <span className={`font-medium ${state.onTime ? "text-success" : "text-warn"}`}>
  ...
  return (
    <div className="mx-auto mb-4 flex max-w-[560px] items-center justify-between gap-3 rounded-2xl border border-flint-blue/20 bg-blue-bg px-4 py-3">
      <div className="min-w-0">
        <div className="font-medium text-ink">{t("gateTitle")}</div>
      ...
      <button
        ...
        onClick={tap}
        disabled={busy}
        className="min-h-11 shrink-0 rounded-full bg-primary px-5 font-mono text-xs font-medium uppercase tracking-widest text-white disabled:opacity-60"
      >
