    ...
    <li className="rounded-2xl border border-black/10 bg-white p-4">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <div className="flex items-center gap-2">
            <span className="font-display font-bold text-flint-black">{m.name}</span>
      ...
      {editing ? (
        <div className="mt-3 grid grid-cols-2 gap-2 border-t border-black/5 pt-3">
          ...
          <button type="button" onClick={() => setEditing(false)} className="min-h-10 rounded-full border border-black/15 font-mono text-xs uppercase tracking-widest text-muted">
            {t("cancel")}
          </button>
        </div>
      ) : (
        <div className="mt-2 flex gap-4">
          <button type="button" onClick={() => setEditing(true)} className="font-mono text-[11px] uppercase tracking-widest text-flint-blue hover:underline">
            {t("staffEdit")}
          </button>
          {!m.isSelf && (
    ...
    <main className="mx-auto max-w-[640px] px-4 pb-16 pt-6">
      <header className="mb-5 flex items-start justify-between gap-4">
        <div>
          ...
          <h1 className="mt-1 font-display text-2xl font-bold text-flint-black">{t("staffNav")}</h1>
      ...
      <section className="rounded-2xl border border-black/10 bg-white p-5">
        <h2 className="mb-3 font-display text-lg font-bold text-flint-black">{t("addStaff")}</h2>
