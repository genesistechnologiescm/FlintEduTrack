...
const area = "min-h-28 w-full rounded-lg border border-black/15 bg-white px-3 py-2 text-base";
    ...
    <main className="mx-auto max-w-[640px] px-4 pb-16 pt-6">
      <header className="mb-5 flex items-start justify-between gap-4">
        <div>
          ...
          <h1 className="mt-1 font-display text-2xl font-bold text-flint-black">{t("resourcesNav")}</h1>
      ...
      {noSubjects ? (
        ...
        <section className="rounded-2xl border border-black/10 bg-white p-5">
          <h2 className="mb-3 font-display text-lg font-bold text-flint-black">{t("addResource")}</h2>
              ...
              {(["LINK", "NOTE"] as const).map((ty) => (
                <button
                  ...
                  className={`min-h-10 flex-1 rounded-full font-mono text-xs uppercase tracking-widest ${
                    type === ty ? "bg-flint-blue text-white" : "border border-black/15 text-muted"
                  }`}
                >
        ...
        <ul className="space-y-3">
          {data.resources.map((r) => (
            <li key={r.id} className="rounded-2xl border border-black/10 bg-white p-5">
              ...
              {r.type === "LINK" && r.url ? (
                <a href={r.url} target="_blank" rel="noopener noreferrer" className="mt-2 inline-block break-all font-mono text-xs text-flint-blue hover:underline">
                  {r.url}
                </a>
              ) : (
