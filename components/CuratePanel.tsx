    ...
    <li className="rounded-2xl border border-line bg-surface p-4">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <div className="font-display font-bold text-ink">{item.title}</div>
          <div className="font-mono text-[11px] text-muted">
            {[item.subject, item.exam, item.submitter, item.school, item.date].filter(Boolean).join(" · ")}
          </div>
        </div>
        <span className="shrink-0 rounded-full bg-chip px-2 py-0.5 font-mono text-[10px] uppercase text-muted">
          {item.kind === "PAST_PAPER" ? t("libPapers") : item.kind === "SYLLABUS" ? t("libSyllabi") : t("libGuides")}
        </span>
      </div>

      {item.url && (
        <a href={item.url} target="_blank" rel="noopener noreferrer" className="mt-2 inline-block break-all font-mono text-xs text-primary hover:underline">
          {item.url}
        </a>
      )}
      {item.body && <p className="mt-2 max-h-48 overflow-y-auto whitespace-pre-wrap border-t border-line pt-2 text-sm text-ink">{item.body}</p>}
