    ...
    <main className="mx-auto max-w-[640px] px-4 pb-16 pt-6">
      <header className="mb-5 flex items-start justify-between gap-4">
        <div>
          ...
          <h1 className="mt-1 font-display text-2xl font-bold text-flint-black">{t("announcementsNav")}</h1>
      ...
      <section className="rounded-2xl border border-black/10 bg-white p-5">
        <h2 className="mb-3 font-display text-lg font-bold text-flint-black">{t("newAnnouncement")}</h2>
        <form onSubmit={onSubmit} className="space-y-3">
          ...
          <label className="block text-sm">
            ...
            <textarea
              className="min-h-28 w-full rounded-lg border border-black/15 bg-white px-3 py-2 text-base"
              value={body}
      ...
      {data.sent.length === 0 ? (
        <p className="rounded-xl border border-black/10 bg-white px-4 py-5 text-center text-muted">{t("noSentYet")}</p>
      ) : (
        <ul className="space-y-3">
          {data.sent.map((a) => (
            <li key={a.id} className="rounded-2xl border border-black/10 bg-white p-5">
              <div className="flex items-start justify-between gap-3">
                <h3 className="font-display font-bold text-flint-black">{a.title}</h3>
              ...
              <div className="mt-3 flex flex-wrap items-center gap-2 border-t border-black/5 pt-3 font-mono text-[11px] text-muted">
                <span className="rounded-full bg-flint-blue/10 px-2 py-0.5 uppercase text-flint-blue">
