...
function StageBadge({ stage }: { stage: Stage }) {
  const { t } = useI18n();
  const map = {
    CONCERN: { label: t("stageConcern"), cls: "bg-flint-blue/10 text-flint-blue" },
    MEETING: { label: t("stageMeeting"), cls: "bg-amber-500/15 text-amber-700" },
    ...
    <li className="rounded-xl border border-black/10 bg-white">
      ...
      >
        <span className="min-w-0">
          <span className="block truncate font-medium text-flint-black">{row.name}</span>
        ...
        <div className="border-t border-black/10 px-4 py-4">
          ...
          {row.events.length === 0 ? (
            ...
            <ul className="space-y-2">
              {row.events.map((e, i) => (
                <li key={i} className="rounded-lg bg-black/5 px-3 py-2 text-sm">
                  <span className="font-mono text-xs text-flint-blue">{evtLabel[e.type]}</span>
                  <span className="ml-2 text-xs text-muted">{e.when}</span>
                  <p className="mt-1 text-flint-black">{e.description}</p>
                </li>
              ))}
            </ul>
          )}
          ...
          <div className="mt-4 space-y-2">
            ...
            <select
              ...
              className="min-h-11 w-full rounded-lg border border-black/15 bg-white px-3 text-sm"
            >
              {EVENT_TYPES.map((tp) => (
            ...
            <button
              ...
              className="inline-flex min-h-11 items-center rounded-full bg-flint-blue px-5 font-mono text-sm font-medium text-white disabled:opacity-60"
            >
  ...
  return (
    <div className="mx-auto max-w-[560px] px-4 pb-16 pt-6">
      <header className="mb-4 flex items-start justify-between gap-4">
        <div>
          <a href="/admin" className="font-mono text-xs text-flint-blue hover:underline">
            {t("backDashboard")}
          </a>
