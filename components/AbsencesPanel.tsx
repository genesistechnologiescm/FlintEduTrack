...
function AbsenceRow({ row }: { row: Row }) {
  ...
  return (
    <li className="rounded-2xl border border-line bg-surface p-4">
      ...
      {open && (
        <form onSubmit={onExcuse} className="mt-3 border-t border-line pt-3">
          <label className="block text-sm">
            <span className="mb-1 block font-mono text-xs uppercase tracking-widest text-muted">{t("excuseReason")}</span>
            <input
              className="min-h-11 w-full rounded-lg border border-line bg-surface px-3 text-base"
              value={reason}
          ...
          <button
            type="submit"
            disabled={busy}
            className="mt-2 min-h-11 w-full rounded-full bg-primary font-mono text-sm font-medium text-white disabled:opacity-60"
          >
