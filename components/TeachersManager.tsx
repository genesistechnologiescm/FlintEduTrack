...
function AssignForm({ teacherUserId, data }: { teacherUserId: string; data: TeachersData }) {
  ...
  return (
    <form onSubmit={submit} className="mt-3 grid grid-cols-2 gap-2 border-t border-black/5 pt-3">
      ...
      <button type="submit" disabled={busy} className="col-span-2 min-h-10 rounded-full border border-flint-blue/30 font-mono text-xs uppercase tracking-widest text-flint-blue disabled:opacity-60">
    ...
    <main className="mx-auto max-w-[640px] px-4 pb-16 pt-6">
      <header className="mb-5 flex items-start justify-between gap-4">
        <div>
          ...
          <h1 className="mt-1 font-display text-2xl font-bold text-flint-black">{t("teachersNav")}</h1>
      ...
      <section className="rounded-2xl border border-black/10 bg-white p-5">
        <h2 className="mb-3 font-display text-lg font-bold text-flint-black">{t("addTeacher")}</h2>
      ...
      <ul className="mt-4 space-y-3">
        {data.teachers.map((tch) => (
          <li key={tch.userId} className="rounded-2xl border border-black/10 bg-white p-5">
            <div className="flex items-center justify-between">
              <span className="font-display font-bold text-flint-black">{tch.name}</span>
